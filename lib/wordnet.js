...
WordNet = (function() {
  ...
  WordNet.prototype.lookup = function(input, callback) {
    ...
    const files = selectedFiles.filter(file => file.index.fileName.includes("noun") || file.index.fileName.includes("adj") || file.index.fileName.includes("adv"));
    var results = [];


    return wordnet.lookupFromFiles(files, results, lword, function(err, results) {
      if (err != null) {
  ...
  WordNet.prototype.pushResultsAsync = async function  (data, offsets)  {
    var results = [];
    if (offsets){
      for (let index = 0; index < offsets.length; index++) {
        var offset = offsets[index];
        var record = await data.getAsync(offset);
        results.push(record);
  ...
  WordNet.prototype.lookupFromFilesAsync = async function  (files, word)  {
    var wordnet;
    wordnet = this;
    var results = [];
    for (let index = 0; index < files.length; index++) {
      ...
      try {
        var record = await file.index.lookupFromFileAsync(word);
        if (record) {
          var records = await wordnet.pushResultsAsync(file.data, record.synsetOffset);
          results.push(records);
          await file.index.closeAsync();
          return results;
        }else{
          return results;
        }
  ...
  WordNet.prototype.lookupAsync = async function  (input)  {
    var wordnet;
    wordnet = this;

    var hit, lword, pos, query, ref, selectedFiles, word;
    ref = input.split('#'), word = ref[0], pos = ref[1];
    lword = word.toLowerCase().replace(/\s+/g, '_');
    if (wordnet.cache) {
      query = "lookup:" + input;
    ...
    var results;
    try {
      results = await wordnet.lookupFromFilesAsync(files, lword);
      if (query) {
        
        wordnet.cache.set(query, results[0]);
        
      }
      return results[0];
    }catch(e){
      console.log("lookupAsync: Error looking up :"+input+" e="+e.message);
    }
  ...
  WordNet.prototype.lookupFromFiles = function(files, results, word, callback) {
    ...
    if (files.length === 0) {
      ...
      file = files.pop();
      return file.index.lookupFromFile(word, function(err, record) {
        if (record) {
